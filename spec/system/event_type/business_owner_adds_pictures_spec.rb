...
describe 'business owner attaches' do
  it 'a picture on a existing event type - w/ no previous pictures' do
  ...
  it 'more than one picture on a existing event type - w/ no previous pictures' do
    kylie = User.create!(name: 'Kylie Kristen Jenner', email: 'khy@jenner.com', password: 'password123', role: 0)
    buffet = Buffet.create!(
      social_name: 'Buffet da Maria',
      corporate_name: 'Buffet da Maria LTDA',
  ...
  it 'a picture on a existing event type - w/ previous pictures' do
    kylie = User.create!(name: 'Kylie Kristen Jenner', email: 'khy@jenner.com', password: 'password123', role: 0)
    buffet = Buffet.create!(
      social_name: 'Buffet da Maria',
      corporate_name: 'Buffet da Maria LTDA',
    ...
    event_type = EventType.create!(
      ...
      per_hour_fee: 100.00,
      per_hour_weekend_fee: 50,
      pictures: [fixture_file_upload(Rails.root.join('spec', 'files', 'reuri.jpeg'))]
    )
    login_as(kylie)
    visit root_path
    ...
    attach_file 'Fotos', Rails.root.join('spec', 'files', 'kevin_malone.jpg')
